...
const contactDisplay = () => {
    ...
    contactNameArray.forEach(name => {
        if(name === "message") {
            let message = "Üzenet";
            string += `<div><label for="${name}">${message}</label><textarea name="${name}" id="${name}" class="input-area message-area"></textarea></div>`;
        }
        else if(name === "button") {
            string += `<div><button type="submit" name="send_message" id="btn" class="contact-btn">Küldés</button></div>`;
        }
        else if(name === "email") {
            let email = "E-mail";
            string += `<div><label for="${name}">${email}</label><input type="email" name="${name}" id="${name}" class="input-area"></div>`;
        }
        else {
            if(name === "name") {
                let formName = "Név";
                string += `<div><label for="${name}">${formName}</label><input type="text" name="${name}" id="${name}" class="input-area"></div>`;
            }
            if(name === "subject") {
                let subject = "Tárgy";
                string += `<div><label for="${name}">${subject}</label><input type="text" name="${name}" id="${name}" class="input-area"></div>`;
            }
        }
    });
    return `
    <article class="article-heading"><h2>Contact</h2></article>
    <article class="contact-h2-text"><form class="contact-form-text">${string}</form></article>
    `;
}
